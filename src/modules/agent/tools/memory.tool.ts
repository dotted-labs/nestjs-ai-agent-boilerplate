...
import { ensureConfiguration } from '../config/configuration';
import { v4 as uuidv4 } from 'uuid';
import { tool } from '@langchain/core/tools';
import { z } from 'zod';
import { getStoreFromConfigOrThrow } from '../config/utils';

/**
 * Initialize tools within a function so that they have access to the current
 * state and config at runtime.
 */
export function initializeMemoryTool(config?: LangGraphRunnableConfig) {
  /**
   ...
   */
  async function upsertMemory(opts: { content: string; context: string; memoryId?: string }): Promise<string> {
